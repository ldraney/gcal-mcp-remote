...
Remote MCP connector wrapping `ldraney-calendar-mcp` with Google OAuth 2.0 over Streamable HTTP.

## Architecture

Three-party OAuth proxy: Claude.ai ↔ this server ↔ Google OAuth.

- **server.py** — Main entrypoint. Imports `mcp` from `calendar_mcp`, configures auth, adds custom routes, runs Streamable HTTP.
...
- **client_patch.py** — Monkey-patches `get_client()` in all `calendar_mcp` tool modules to use a per-request `ContextVar`-based `GCalClient`.

## Key Patterns

- The `mcp` FastMCP instance is imported from `calendar_mcp` and reconfigured at runtime (auth settings, host/port, stateless mode).
...
- All 3 tool modules (`events`, `calendars`, `freebusy`) import `get_client` from `calendar_mcp.server` — the patch replaces it in each module's namespace.
